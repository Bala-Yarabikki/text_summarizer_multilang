...
# Reading Time
def reading_time(my_text):
    lang = detect(my_text)
    if lang in dict_model_nlp:
        nlp = dict_model_nlp[lang]
    else:
        nlp = nlp_tt
    total_words = len([token.text for token in nlp(my_text)])
...
def comparer():
    ...
    return render_template('compare_summary.html',ctext=rawtext,final_summary_spacy=final_summary_spacy,
                           ...
                           summary_reading_time=summary_reading_time,summary_reading_time_gensim=summary_reading_time_gensim,
